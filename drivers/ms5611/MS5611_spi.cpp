 ...
 *      Author: Fan.zhang  421395590@qq.com
 */
...
int64_t MS5611_spi::convertPressure(int64_t adc_P)
{
...
int32_t MS5611_spi::convertTemperature(int32_t adc_T)
{
	// Conversion from the datasheet
...
*/
bool MS5611_spi::crc4(uint16_t *n_prom)
{
	int16_t cnt;
	uint16_t n_rem;
	uint16_t crc_read;
...
int MS5611_spi::loadCalibration()
{
...
int MS5611_spi::ms5611_spi_init()
{
	/* Zero the struct */
	m_synchronize.lock();
...
int MS5611_spi::start()
{
	...
	_setBusFrequency(SPI_FREQUENCY_1MHZ); //使用1MHZ作为通信频率

	/* Initialize the pressure sensor.*/
	result = ms5611_spi_init();
...
int MS5611_spi::_request(uint8_t cmd)
{
	int ret;
	_retries = 0;
...
int MS5611_spi::_collect(uint32_t &raw)
{
	int ret;
...
int MS5611_spi::_publish(struct baro_sensor_data &data)
{
	// TBD
	return -1;
}
